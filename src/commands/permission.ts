...
export async function execute(interaction: ChatInputCommandInteraction) {
    ...
    if (!interaction.guildId) {
        return interaction.reply({ content: "Vous devez être dans un serveur pour utiliser cette commande.", flags: ["Ephemeral"] })
    }
    ...
    try {
        permissions = getPermissions(guildId);
    } catch (error) {
    ...
    switch (action) {
        case "grant":
            ...
            if (!permissions[command].includes(role.id)) {
                ...
                savePermissions(permissions, guildId);
            }
